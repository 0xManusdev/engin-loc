...
import { Menu, Search, ShoppingCart, MessageCircle, User, Building, ChevronDown, LogIn } from "lucide-react"
...
export function SiteHeader() {
	...
	return (
		<header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
			...
			<div className="container flex flex-row justify-between h-16 items-center">
				<div className="flex items-center">
					<Link href="/" className="flex items-center space-x-2 mr-4">
						<span className="font-bold text-xl text-accent">K&R Secure</span>
					</Link>
				</div>

				{/* menu desktop qui doit être au centre */}
					...
					<nav className="flex items-center gap-6">
						<DropdownMenu>
							...
							<DropdownMenuContent align="start" className="w-56">
								...
								<DropdownMenuItem asChild>
									<Link href="/client/catalogue">Voir tout</Link>
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="start" className="w-56">
								<DropdownMenuItem asChild>
									<Link href="/client/vente-achat">Vente & Achat</Link>
								</DropdownMenuItem>
								<DropdownMenuItem asChild>
									<Link href="/client/service-apres-vente">Service Après-Vente</Link>
								</DropdownMenuItem>
								<DropdownMenuItem asChild>
									<Link href="/client/surete-securite">Sûreté & Sécurité</Link>
								</DropdownMenuItem>
								<DropdownMenuItem asChild>
									<Link href="/client/conseils-assistance">Conseils & Assistance</Link>
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>

						<Link href="/client/devenir-partenaire" className="text-sm font-medium transition-colors hover:text-primary">
							Devenir Partenaire
						</Link>
						<Link href="/client/contact" className="text-sm font-medium transition-colors hover:text-primary">
							Contact
						</Link>
					</nav>
				...
				<div className="flex items-center gap-4">
					...
					<Link href="/connexion" className="flex items-center">
						<Button variant="outline" className="rounded-full bg-accent hover:bg-accent/80">
							<span className="hidden text-white md:block">Connexion</span>
						</Button>
					</Link>

					<Link href="/inscription" className="flex items-center">
						<Button variant="outline" className="rounded-full hover:bg-accent/80">
							<span className="hidden hover:text-white md:block">Inscription</span>
						</Button>
					</Link>
				</div>
